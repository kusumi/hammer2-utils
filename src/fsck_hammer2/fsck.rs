...
impl BlockrefMessage {
    fn new(bref: &hammer2fs::Hammer2Blockref) -> Self {
        Self {
            bref: *bref,
            msg: [0; 1024],
        }
    ...
    fn new_from_str(bref: &hammer2fs::Hammer2Blockref, s: &str) -> Self {
        let mut m = BlockrefMessage::new(bref);
        m.msg[..s.len()].copy_from_slice(s.as_bytes());
        m
    }

    fn new_from<T>(bref: &hammer2fs::Hammer2Blockref, x: &T) -> Self
    where
        T: Clone + Copy,
    {
        let mut m = BlockrefMessage::new(bref);
        *util::align_head_to_mut(&mut m.msg) = *x;
        m
    }

    fn msg_as<T>(&self) -> &T {
        util::align_head_to(&self.msg)
    }
}
...
fn alloc_root_blockref(i: usize, typ: u8) -> hammer2fs::Hammer2Blockref {
    assert!(
        typ == hammer2fs::HAMMER2_BREF_TYPE_EMPTY
            || typ == hammer2fs::HAMMER2_BREF_TYPE_FREEMAP
            || typ == hammer2fs::HAMMER2_BREF_TYPE_VOLUME
    );
    let mut bref = hammer2fs::Hammer2Blockref::new(typ);
    bref.data_off = (u64::try_from(i).unwrap() * hammer2fs::HAMMER2_ZONE_BYTES)
        | u64::try_from(hammer2fs::HAMMER2_PBUFRADIX).unwrap();
    bref
}
...
fn test_volume_header(
    ...
    for i in 0..hammer2fs::HAMMER2_NUM_VOLHDRS {
        ...
        if offset < vol.get_size() {
            let broot = alloc_root_blockref(i, hammer2fs::HAMMER2_BREF_TYPE_EMPTY);
            print_zone_summary(0, i, zone, &broot, opt);
            let buf = vol.preadx(hammer2fs::HAMMER2_VOLUME_BYTES, offset)?;
            if let Err(e) = verify_volume_header(util::align_to(&buf)) {
                if failure.is_none() {
                    failure = Some(e);
                }
            }
...
fn test_pfs_blockref(
    ...
    for i in 0..hammer2fs::HAMMER2_NUM_VOLHDRS {
        ...
        let offset = volume::get_volume_data_offset(i);
        if offset < vol.get_size() {
            let typ = hammer2fs::HAMMER2_BREF_TYPE_VOLUME;
            let broot = alloc_root_blockref(i, typ);
            print_zone_summary(0, i, zone, &broot, opt);
            ...
            for m in &blist {
                let mut found = false;
                let ipdata = m.msg_as::<hammer2fs::Hammer2InodeData>();
...
fn add_blockref_entry_from_str(root: &mut BlockrefMap, bref: &hammer2fs::Hammer2Blockref, s: &str) {
    add_blockref_entry_impl(root, bref, BlockrefMessage::new_from_str(bref, s));
}

fn add_blockref_entry<T>(root: &mut BlockrefMap, bref: &hammer2fs::Hammer2Blockref, x: &T)
where
    T: Clone + Copy,
{
...
fn format_blockref(tab: usize, bref: &hammer2fs::Hammer2Blockref, msg: &str) -> String {
    tab::format!(
        tab,
        "{:016x} {:<12} {:016x}/{:<2}{}{}",
        bref.data_off,
        subs::get_blockref_type_string(bref.typ),
        bref.key,
        bref.keybits,
...
fn print_blockref_debug(bref: &hammer2fs::Hammer2Blockref, msg: &str, opt: &Hammer2FsckOptions) {
    if opt.debug {
...
fn print_blockref_entry(
    ...
    for e in root.values() {
        for m in e {
            eprintln!(
                "{}",
                format_blockref(1, &m.bref, &util::bin_to_string(&m.msg).unwrap())
            );
            if opt.verbose {
...
fn print_blockref_stats(
    ...
    let buf = match bstats.typ {
        hammer2fs::HAMMER2_BREF_TYPE_FREEMAP => tab::format!(
            1,
            "{} blockref ({} node, {} leaf{}), {}",
            ...
            emptybuf,
            subs::get_size_string(bstats.total_bytes)
        ),
        hammer2fs::HAMMER2_BREF_TYPE_VOLUME => tab::format!(
            1,
    ...
    let buf = if let Some(v) = buf.get(..subs::get_chars_per_line()) {
        v.to_string()
    } else {
        buf
    };
...
fn verify_volume_header(voldata: &hammer2fs::Hammer2VolumeData) -> std::io::Result<()> {
    if voldata.magic != hammer2fs::HAMMER2_VOLUME_ID_HBO
        && voldata.magic != hammer2fs::HAMMER2_VOLUME_ID_ABO
    {
        tab::eprintln!(1, "Bad magic {:x}", voldata.magic);
        return Err(std::io::Error::from(std::io::ErrorKind::InvalidInput));
    }
    if voldata.magic == hammer2fs::HAMMER2_VOLUME_ID_ABO {
...
fn verify_blockref(
    ...
    match bref.typ {
        ...
        hammer2fs::HAMMER2_BREF_TYPE_DATA => {
        ...
        hammer2fs::HAMMER2_BREF_TYPE_FREEMAP_NODE => {
            bstats.freemap.total_freemap_node += 1;
            dstats.freemap.total_freemap_node += 1;
        }
        hammer2fs::HAMMER2_BREF_TYPE_FREEMAP_LEAF => {
    ...
    if bref.typ != hammer2fs::HAMMER2_BREF_TYPE_VOLUME
        && bref.typ != hammer2fs::HAMMER2_BREF_TYPE_FREEMAP
    {
        bstats.total_bytes += bytes;
        dstats.total_bytes += bytes;
    }
    if !opt.count_empty && bref.typ == hammer2fs::HAMMER2_BREF_TYPE_EMPTY {
        assert_eq!(bytes, 0);
        bstats.total_bytes -= bytes;
        dstats.total_bytes -= bytes;
    }
    ...
    if bytes != 0 {
        match hammer2fs::dec_check(bref.methods) {
            hammer2fs::HAMMER2_CHECK_ISCSI32 => {
                let cv = icrc32::iscsi_crc32(&media);
                if bref
                    .check_as::<hammer2fs::Hammer2BlockrefCheckIscsi>()
                    .value
                    != cv
                {
            ...
            hammer2fs::HAMMER2_CHECK_XXHASH64 => {
                ...
                if bref
                    .check_as::<hammer2fs::Hammer2BlockrefCheckXxhash64>()
                    .value
                    != cv
                {
            ...
            hammer2fs::HAMMER2_CHECK_FREEMAP => {
                let cv = icrc32::iscsi_crc32(&media);
                if bref
                    .check_as::<hammer2fs::Hammer2BlockrefCheckFreemap>()
                    .icrc32
                    != cv
                {
...
fn print_pfs(ipdata: &hammer2fs::Hammer2InodeData) {
    let meta = &ipdata.meta;
    let type_str = if meta.pfs_type == hammer2fs::HAMMER2_PFSTYPE_MASTER {
        if meta.pfs_subtype == hammer2fs::HAMMER2_PFSSUBTYPE_NONE {
            "MASTER".to_string()
        } else {
            subs::get_pfs_subtype_string(meta.pfs_subtype)
        }
    } else {
        subs::get_pfs_subtype_string(meta.pfs_type)
    };
    tab::println!(
        1,
        "{type_str:<11} {} {}",
...
fn scan_pfs_blockref(
    ...
    let bscan = match bref.typ {
        hammer2fs::HAMMER2_BREF_TYPE_INODE => {
            let ipdata = util::align_to::<hammer2fs::Hammer2InodeData>(&media);
            if ipdata.meta.pfs_type == hammer2fs::HAMMER2_PFSTYPE_SUPROOT {
                ipdata
                    .u_as::<hammer2fs::Hammer2Blockset>()
                    .as_blockref()
                    .to_vec()
            } else {
                if ipdata.meta.op_flags & hammer2fs::HAMMER2_OPFLAG_PFSROOT != 0 {
                    v.push(BlockrefMessage::new_from(bref, ipdata));
                    v.sort_by_key(|m| m.msg_as::<hammer2fs::Hammer2InodeData>().filename);
                } else {
        ...
        hammer2fs::HAMMER2_BREF_TYPE_INDIRECT => extra::media_as(&media),
        hammer2fs::HAMMER2_BREF_TYPE_VOLUME => {
            util::align_to::<hammer2fs::Hammer2VolumeData>(&media)
                .sroot_blockset
                .as_blockref()
...
fn format_media(tab: usize, bref: &hammer2fs::Hammer2Blockref, media: &[u8]) -> Vec<String> {
    let mut v = vec![];
    match bref.typ {
        hammer2fs::HAMMER2_BREF_TYPE_INODE => {
            let ipdata = util::align_to::<hammer2fs::Hammer2InodeData>(media);
            ...
            v.push(tab::format!(
                tab,
                "ctime {}\n",
                subs::get_local_time_string(meta.ctime)
            ));
            v.push(tab::format!(
                tab,
                "mtime {}\n",
                subs::get_local_time_string(meta.mtime)
            ));
            v.push(tab::format!(
                tab,
                "atime {}\n",
                subs::get_local_time_string(meta.atime)
            ));
            v.push(tab::format!(
                tab,
                "btime {}\n",
                subs::get_local_time_string(meta.btime)
            ));
            v.push(tab::format!(
                tab,
                "uid {}\n",
            ...
            v.push(tab::format!(
                tab,
                "type {}\n",
                subs::get_inode_type_string(meta.typ)
            ));
        ...
        hammer2fs::HAMMER2_BREF_TYPE_INDIRECT | hammer2fs::HAMMER2_BREF_TYPE_FREEMAP_NODE => {
            for (i, bref) in extra::media_as::<hammer2fs::Hammer2Blockref>(media)
                .iter()
                .enumerate()
            {
                v.push(tab::format!(
        ...
        hammer2fs::HAMMER2_BREF_TYPE_DIRENT => {
            ...
            v.push(tab::format!(
                tab,
                "type {}\n",
                subs::get_inode_type_string(dirent.typ)
            ));
        }
        hammer2fs::HAMMER2_BREF_TYPE_FREEMAP_LEAF => {
...
pub(crate) fn fsck(devpath: &str, opt: &Hammer2FsckOptions) -> std::io::Result<()> {
    ...
    println!("freemap");
    if let Err(e) = test_blockref(&mut fso, hammer2fs::HAMMER2_BREF_TYPE_FREEMAP, zone, opt) {
        if !opt.force {
            return Err(e);
        }
    }
    println!("volume");
    if !opt.scan_pfs {
        if let Err(e) = test_blockref(&mut fso, hammer2fs::HAMMER2_BREF_TYPE_VOLUME, zone, opt) {
            if !opt.force {
                return Err(e);
            }
        }
...
mod tests {
    ...
    fn test_blockref_message_new_from_str() {
        ...
        for s in [String::new(), "A".to_string(), "A".repeat(1024)] {
            ...
            assert_eq!(super::util::bin_to_string(&m.msg).unwrap(), s);
        }
    }
    ...
    fn test_blockref_message_new_from() {
        ...
        let m = super::BlockrefMessage::new_from(&bref, &super::hammer2fs::Hammer2InodeData::new());
        eq!(m.msg, m.msg_as::<super::hammer2fs::Hammer2InodeData>());
    }
}
